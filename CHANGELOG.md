...
- change res/app.rc and res/app.ico for ulord
- change src/xmrig.h
	-- change CRYPTONIGHT to CRYPTOHELLO
- change config.json
	- change mode from nicehash to simple, cannot support nicehash this time
	- change url to ulord pool
	- change user to ulord wallet
- change src/net/Job.h
	- change 'm_blob + 39 to 'm_blob + 108'
	- change m_blob size, ulord is 140 bytes
	- change m_rawBlob size, ulord is 280 chars
- change src/net/Job.cpp
	- change setBlob accrording to ulordrig's Job.cpp
- change src/proxy/LoginRequest.cpp
	- add 'Ulordrig' agent in detectClient()
-- change src/proxy/JobReqult.cpp
	- ulord no need to check fixedByte in  isCompatible()
